...
function EducationBlock({ value, handleInput }) {
	...
	return (
		<div className="m-2 grid grid-cols-3 grid-rows-2">
			...
			required></input>
            <div>
			<label className="mx-2" for="start">Start date:</label>
			...
			required></input></div>
            <div>
			<label className="mx-2"  for="end">End date:</label>
			<input className="ml-2 pl-2 inline-block rounded border border-gray-200 block  placeholder-black focus:placeholder-gray-400 hover:border-blue-500 focus:border-blue-500 focus:border-black focus:outline-none focus:ring focus:border-blue-300"
				type="date"
				id="end"
				name="education-end"
				value={formatDate(new Date())}
